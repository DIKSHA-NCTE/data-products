...
#base_dir = '/home/ramya/NEW_druid_scripts/Diksha_courses/output/batch_wise/All_Tenant_report_' + Date + '.csv'
...
def analysis_work():
  ...
  "limitSpec": {
    ...
    try:
        ...
        df_snapshot = df_source.select(F.col('content_org').alias('Published by'),'collection_id', 'collection_name', 'batch_id', 'batch_start_date','batch_end_date', 'has_certificate',F.col('SUM(total_enrolment)').alias('Total_Enrollment'),F.col('SUM(total_completion)').alias('Total_Completion'),F.col('SUM(total_certificates_issued)').alias('Total_Certifications')).toPandas()
        df_snapshot.info()
        ...
        daily_snapshot = df_snapshot[['Published by', 'collection_id', 'collection_name', 'batch_id', 'batch_start_date', 'batch_end_date',
         'Total_Enrollment', 'Total_Completion', 'has_certificate', 'Total_Certifications', 'Cert_Lag']]

        daily_snapshot.columns = ['tenant_name', 'course_id', 'course_name', 'batch_id', 'start_date', 'end_date',
                              'Total_Enrollment', 'Total_Completion', 'Has certificate', 'Total_Certifications',
                              'Cert_Lag']
    ...
    tab_name = 'Batch_wise_report-Druid'
