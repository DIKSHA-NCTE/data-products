...
from dataproducts.util.utils import create_json, get_batch_data_from_blob, get_content_plays, get_content_model, \
    get_data_from_blob, get_tenant_info, get_tb_content_mapping, post_data_to_blob, push_metric_event
...
class ContentConsumption:
    ...
    def get_overall_report(result_loc_, druid_rollup_, date_, config):
        ...
        overall.columns = ['channel', 'Board', 'Medium', 'Grade', 'Subject', 'Content ID', 'Content Name',
                           'Mime Type', 'Created On', 'Creator (User Name)', 'verticals', 'programs',
                           'Last Published On',
    ...
    def init(self):
        ...
        get_content_model(result_loc_=result_loc, druid_=druid_rollup, date_=execution_date, config_=self.config,
                          version_='v2')
